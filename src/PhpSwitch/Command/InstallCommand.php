...
use PhpSwitch\Tasks\AfterConfigureTask;
use PhpSwitch\Tasks\BeforeConfigureTask;
use PhpSwitch\Tasks\BuildTask;
use PhpSwitch\Tasks\ConfigureTask;
use PhpSwitch\Tasks\DownloadTask;
use PhpSwitch\Tasks\DSymTask;
use PhpSwitch\Tasks\ExtractTask;
use PhpSwitch\Tasks\InstallTask;
use PhpSwitch\Tasks\MakeTask;
use PhpSwitch\Tasks\PrepareDirectoryTask;
use PhpSwitch\Tasks\TestTask;
...
class InstallCommand extends Command
{
    ...
    {
        return 'phpswitch install [php-version] ([+variant...])';
    }
    ...
    public function options($opts)
    {
        ...
        $opts->add('build-dir:', 'Specify the build directory. '
            ...
            . 'instead of $PHPSWITCH_ROOT/build/{name}.')
            ->isa('dir')
            ;

        $opts->add('root', 'Specify Phpswitch root instead of PHPSWITCH_ROOT');

        $opts->add('home', 'Specify Phpswitch home instead of PHPSWITCH_HOME');
        ...
        $opts->add('no-clean', 'Do not clean previously compiled objects before building PHP. '
            . 'By default phpswitch will run `make clean` before running the configure script '
        ...
        $opts->add('old', 'Install phpswitch incompatible phps (< 5.3)');
    ...
    public function execute($version)
    {
        if (extension_loaded('posix') && posix_getuid() === 0) {
            $this->logger->warn(
                "*WARNING* You're running phpswitch as root/sudo. Unless you're going to install "
                . "system-wide phpswitch, this might cause problems."
            );
            sleep(3);
        }
        $distUrl = null;
        ...
        $msg = "===> phpswitch will now build {$build->getVersion()}";
        if ($buildLike) {
        ...
        chdir($targetDir);
        // Write variants info.
        $variantInfoFile = $build->getInstallPrefix() . DIRECTORY_SEPARATOR . 'phpswitch.variants';
...
* WARNING:
  You haven't run 'phpswitch init' yet! Be sure to setup your phpswitch to use your own php(s)
  Please run 'phpswitch init' to setup your phpswitch in place.
...
EOT;
        ...
        if (!getenv('PHPSWITCH_HOME')) {
            echo <<<EOT

* WARNING:
  You haven't setup your .bashrc file to load phpswitch shell script yet!
  Please run 'phpswitch init' to see the steps!
